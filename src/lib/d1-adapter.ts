...
// D1 allows reusing ?1, ?2 multiple times; better-sqlite3 uses positional ?.
// We normalize SQL and expand the bind params to match positional order.
function normalizeParams(sql: string): string {
  return sql.replace(/\?(\d+)/g, '?');
}

// Build expanded params array: for each ? in normalized SQL, pick the param
// from the original bind array based on the numbered position in the source SQL.
function expandParams(sql: string, params: unknown[]): unknown[] {
  const paramIndices: number[] = [];
  const numbered = /\?(\d+)/g;
  let m: RegExpMatchArray | null;
  while ((m = numbered.exec(sql)) !== null) {
    paramIndices.push(parseInt(m[1], 10) - 1); // ?1 maps to index 0
  }
  // If no numbered params found, return original params as-is
  if (paramIndices.length === 0) return params;
  // Expand: each occurrence maps to the original param at that index
  return paramIndices.map(i => params[i]);
}
...
export function createD1Adapter(dbPath: string): D1Database {
  ...
  return {
    prepare(sql: string): D1PreparedStatement {
      ...
      return {
        bind(...params: unknown[]) {
          return makeBindResult(expandParams(sql, params));
        },
